...
class UsuarioManager extends GenericManager {
	...
	public function save($usuario) {
		...
		$query = "INSERT INTO usuario (nom_usuario_gpsgate, dsc_nom_usuario, dsc_email_usuario, dsc_senha_usuario, dsc_endereco, dsc_complemento, dsc_cep, dsc_pais, dsc_uf, dsc_cidade, num_telefone, num_fone_celular, cod_cpf_cnpj, dat_nascimento, sta_admin, num_app_gpsgate) VALUES ('".$usuario->nomUsuarioGpsgate."', '".$usuario->dscNomUsuario."', '".$usuario->dscEmailUsuario."', '".$usuario->dscSenhaUsuario."', '".$usuario->dscEndereco."', '".$usuario->dscComplemento."', '".$usuario->dscCep."', '".$usuario->dscPais."', '".$usuario->dscUf."', '".$usuario->dscCidade."', '".$usuario->numTelefone."', '".$usuario->numFoneCelular."', '".$usuario->codCpfCnpj."', timestamp(str_to_date('".$usuario->datNascimento."', '%d/%m/%Y')), ".$usuario->staAdmin.", ".$usuario->numAppGpsgate.")";
   ...
   " num_app_gpsgate = ".$usuario->numAppGpsgate." WHERE seq_usuario = ".$usuario->seqUsuario ;
   ...
   //" dat_ultimo_login = timestamp(str_to_date('".$usuario->datUltimoLogin."', '%d/%m/%Y') WHERE seq_usuario = ".$usuario->seqUsuario ;
	...
	public function findById($idUsuario) {
		...
		while($item = mysql_fetch_array($result)) 
		{
			...
			$dataNascimento = date_create($item['dat_nascimento']);
			  
			$usuario->datNascimento = date_format($dataNascimento, 'd/m/Y');   
			$usuario->staAdmin = $item['sta_admin'];   
			$usuario->numAppGpsgate = $item['num_app_gpsgate']; 
			
			$dataUltimoLogin = date_create($item['dat_ultimo_login']);
			  
			$usuario->datUltimoLogin = date_format($dataUltimoLogin, 'd/m/Y');    
		}
	...
	public function findAll() {
		...
		$query = "SELECT *, dat_nascimento FROM usuario";
	...
	public function countUsuarios() {
		...
		$query = "SELECT 
   				 (select count(*) from usuario where sta_admin = 1) as usu_admin, 
                 (select count(*) from usuario where sta_admin = 2) as usu_app";

		return $this->executeQuery($query);

	}
	
	public function findUsuariosAdmin() {

		$query = "SELECT * FROM usuario WHERE sta_admin = 1";
